...
import { getAllLocalDocsets, getAllDocsets, getDashIndexFilePath } from './helpers/parseDocset'
...
app.get('/docs/:libname', async (req: Request, res: Response) => {
    ...
    const docsetFileName = docsets.find((docset) => {
        ...
        return docsetLower.includes(libnameEncoded) && docsetLower.endsWith('.docset')
    });
    ...
    if (!docsetFileName) {
        ...
        if (xmlFileName) {
            return res.status(200).send('Found xml')
        }
    ...
    const indexPath = path.join(DOCS_DIR, docsetFileName, 'Contents/Resources/Documents', docsetPath)
...
app.get('/files/*', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!filePath.startsWith(path.join(__dirname, '..', 'docsets')) || filePath.includes('..')) {
...
app.get("/api/remote-docs", async (_req, res) => {
    try {
